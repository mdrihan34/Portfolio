import { FaHtml5, FaCss3Alt, FaJs, FaReact, FaUserCheck, FaGithub, FaGitAlt} from 'react-icons/fa';
import { SiRedux } from 'react-icons/si';
import { RiTailwindCssFill } from "react-icons/ri";
import { IoLogoNodejs } from 'react-icons/io';
import { BiLogoMongodb } from 'react-icons/bi';
import SkillBg from '../../public/image/skills.png'
const Skills = () => {
  return (
    <div className='py-10'  style={{ backgroundImage: `url(${SkillBg})` , backgroundRepeat : 'no-repeat', backgroundSize : 'cover'}}>
      
      <div className="text-center py-8">
      <h2 className="text-4xl font-bold text-white uppercase relative inline-block">
      My Skills
        <span className="block h-1 w-16 bg-blue-500 mx-auto mt-2"></span>
      </h2>
    </div>
     <div className="lg:flex md:flex gap-10 justify-center lg:px-44">
      <div className="shadow-xl rounded-xl w-full  p-8 text-white">
        <div className='text-center'>
        <h1 className='text-2xl font-bold text-orange-500'>Frontend Development</h1>
        <p>Crafting user-friendly interfaces.</p>
        </div>
        <div>
        <div className="container relative z-40 mx-auto mt-12">

<div className="flex lg:grid   lg:gap-8 lg:grid-cols-3 lg:ml-[60px]  flex-wrap justify-center  lg:w-full md:w-5/6 xl:shadow-small-blue">

    <a href="#" className="block boxs w-1/2 py-10  rounded-xl lg:py-4 text-center  lg:w-32 lg:h-32">
        <div>
          <FaHtml5 className="block mx-auto text-5xl text-sky-400"></FaHtml5>
           

            <p className="pt-4 text-sm font-medium capitalize  font-body text-yellow-500 lg:font-bold lg:text-lg md:text-base md:pt-6">
              HTML
            </p>
        </div>
    </a> 

    <a href="#" className="block boxs w-1/2 py-10  rounded-xl lg:py-4 text-center  lg:w-32 lg:h-32">
        <div>
          <FaCss3Alt className="block mx-auto text-5xl text-sky-400"></FaCss3Alt>
           

            <p className="pt-4 text-sm font-medium capitalize  font-body text-yellow-500 lg:font-bold lg:text-lg md:text-base md:pt-6">
              CSS
            </p>
        </div>
    </a> 

    <a href="#" className="block boxs w-1/2 py-10  rounded-xl lg:py-4 text-center  lg:w-32 lg:h-32">
        <div>
          <FaJs className="block mx-auto text-5xl text-sky-400"></FaJs>
           

            <p className="pt-4 text-sm font-medium capitalize  font-body text-yellow-500 lg:font-bold lg:text-lg md:text-base md:pt-6">
            JavaScript
            </p>
        </div>
    </a> 

    <a href="#" className="block boxs w-1/2 py-10  rounded-xl lg:py-4 text-center  lg:w-32 lg:h-32">
        <div>
          <FaReact className="block mx-auto text-5xl text-sky-400"></FaReact>
           

            <p className="pt-4 text-sm font-medium capitalize  font-body text-yellow-500 lg:font-bold lg:text-lg md:text-base md:pt-6">
              React
            </p>
        </div>
    </a> 

    <a href="#" className="block boxs w-1/2 py-10  rounded-xl lg:py-4 text-center  lg:w-32 lg:h-32">
        <div>
          <RiTailwindCssFill className="block mx-auto text-5xl text-sky-400"></RiTailwindCssFill>
           

            <p className="pt-4 text-sm font-medium capitalize  font-body text-yellow-500 lg:font-bold lg:text-lg md:text-base md:pt-6">
            Tailwind
            </p>
        </div>
    </a> 

   

 
</div>

</div>
        </div>
      </div>
      <div className="shadow-xl w-full rounded-xl  p-8 text-white">
     <div className='text-center'>
     <h1 className='text-2xl font-bold text-orange-500'>Backend Development</h1>
     <p>Building scalable backends.</p>
     </div>
     <div>
     <div className="container relative z-40 mx-auto mt-12">

<div className="flex lg:grid   lg:gap-8 lg:grid-cols-3 lg:ml-[60px]  flex-wrap justify-center  lg:w-full md:w-5/6 xl:shadow-small-blue">

    <a href="#" className="block boxs w-1/2 py-10  rounded-xl lg:py-4 text-center  lg:w-32 lg:h-32">
        <div>
          <IoLogoNodejs className="block mx-auto text-5xl text-sky-400"></IoLogoNodejs>
           

            <p className="pt-4 text-sm font-medium capitalize  font-body text-yellow-500 lg:font-bold lg:text-lg md:text-base md:pt-6">
              Node.js
            </p>
        </div>
    </a> 
    <a href="#" className="block boxs w-1/2 py-10  rounded-xl lg:py-4 text-center  lg:w-32 lg:h-32">
        <div>
          <BiLogoMongodb className="block mx-auto text-5xl text-sky-400"></BiLogoMongodb>
           

            <p className="pt-4 text-sm font-medium capitalize  font-body text-yellow-500 lg:font-bold lg:text-lg md:text-base md:pt-6">
            Mongodb
            </p>
        </div>
    </a>

    <a href="#" className="block boxs w-1/2 py-10  rounded-xl lg:py-4 text-center  lg:w-32 lg:h-32">
        <div>
        
          <img className="block mx-auto w-12 text-sky-400" src="https://i.postimg.cc/G2SC3bVY/express-js.png" alt="" />
           

            <p className="pt-4 text-sm font-medium capitalize  font-body text-yellow-500 lg:font-bold lg:text-lg md:text-base md:pt-6">
            Express.js
            </p>
        </div>
    </a> 

  
   

    <a href="#" className="block boxs w-1/2 py-10 px-4 rounded-xl lg:py-4 text-center  lg:w-36 lg:h-32">
        <div>
          <FaUserCheck className="block mx-auto text-5xl text-sky-400"></FaUserCheck>
           

            <p className="pt-4 text-sm font-medium capitalize  font-body text-yellow-500 lg:-ml-2   md:text-base md:pt-6">
            Authentication
            </p>
        </div>
    </a> 



 
</div>

</div>
     </div>
      </div>
     </div>
      <div className='lg:px-[17%]'>
        <div className="shadow-xl rounded-xl lg:w-1/2 lg:mx-80 mt-10  p-8 text-white">
         <h1 className='text-2xl font-bold text-orange-500 text-center'>Version Control </h1>
         <div className="container relative z-40 mx-auto mt-12">
          <div className="flex gap-2  lg:gap-8 lg:grid-cols-3 flex-wrap justify-center  lg:w-full md:w-5/6 xl:shadow-small-blue">
          <a href="#" className="block boxs w-1/2 py-10  rounded-xl lg:py-4 text-center  lg:w-32 lg:h-32">
        <div>
          <FaGithub className="block mx-auto text-5xl text-sky-400"></FaGithub>
           

            <p className="pt-4 text-sm font-medium capitalize  font-body text-yellow-500 lg:font-bold lg:text-lg md:text-base md:pt-6">
              Github
            </p>
        </div>
    </a> 
    <a href="#" className="block boxs w-1/2 py-10  rounded-xl lg:py-4 text-center  lg:w-32 lg:h-32">
        <div>
          <FaGitAlt className="block mx-auto text-5xl text-sky-400"></FaGitAlt>
           

            <p className="pt-4 text-sm font-medium capitalize  font-body text-yellow-500 lg:font-bold lg:text-lg md:text-base md:pt-6">
              Git
            </p>
        </div>
    </a> 
          </div>

         </div>
        </div>
      </div>
    </div>
  )
}

export default Skills
